...
public final class Deadline implements Comparable<Deadline> {
   ...
   * stabilized or have any feedback, please <a
   * href="https://github.com/grpc/grpc-java/issues/6030">let us know</a>.
   *
   * @since 1.24.0
   */
  ...
  public static Deadline after(long duration, TimeUnit units, Ticker ticker) {
    ...
    return new Deadline(ticker, units.toNanos(duration), /* baseInstantAlreadyExpired= */ true);
  }
  ...
  private Deadline(Ticker ticker, long offset, boolean baseInstantAlreadyExpired) {
    this(ticker, offset, baseInstantAlreadyExpired, ticker.nanoTime());
  }

  private Deadline(
      Ticker ticker, long offset, boolean baseInstantAlreadyExpired, long baseInstant) {
    this.ticker = ticker;
  ...
  /**
   * Returns {@code true} if this deadline has expired.
   *
  ...
  public Deadline offset(long offset, TimeUnit units) {
    ...
    return new Deadline(ticker, units.toNanos(offset), isExpired(), deadlineNanos);
  }

  /**
   * How much time is remaining in the specified time unit. Internal units are maintained as
   * nanoseconds and conversions are subject to the constraints documented for {@link
   * TimeUnit#convert}. If there is no time remaining, the returned duration is how long ago the
   * deadline expired.
   */
   ...
   * with custom tickers are incompatible with those created with the system ticker. Always use the
   * {@link #getSystemTicker system ticker} whenever you need to provide one in production code.
   *
   * <p>This is <strong>EXPERIMENTAL</strong> API and may subject to change. If you'd like it to be
   * stabilized or have any feedback, please <a
   * href="https://github.com/grpc/grpc-java/issues/6030">let us know</a>.
